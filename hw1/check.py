...
##### Enter Weights Here ###########
            scale = 10
            in_hid_weight  = [[1 * scale, 1 * scale],[1 * scale, -1 * scale],[-1 * scale, 1 * scale],[-1 * scale, -1 * scale]]
            hid_bias       = [0.5 * scale, 0.5 * scale, 0.5 * scale, 0.5 * scale]
            hid_out_weight = [[1 * scale, 1 * scale, 1 * scale, 1 * scale]]
            out_bias       = [-2.7 * scale]
####################################
            self.in_hid.weight.data = torch.tensor(
                 in_hid_weight, dtype=torch.float32)
